...
if not (os.path.exists("File_Handling/Notes_app/Notes")):
    os.makedirs("File_Handling/Notes_app/Notes")
    ...
    if action == "1":
        ...
        if write_mode_choice == "1":
            ...
            if title.lower().strip() == "e" or note.lower().strip() == "e":
                break
            if os.path.exists(f"File_Handling/Notes_app/Notes/{title}.txt"):
                print("File already exists.")
            else:
                with open(f"File_Handling/Notes_app/Notes/{title.replace(' ', '_')}.txt", "w") as f:
                    f.write(note)
                print("Note written")
            ...
            if title.lower().strip() == "e" or note.lower().strip() == "e":
                break
            if not (os.path.exists(f"File_Handling/Notes_app/Notes/{title}")):
                print("File doesnt exist.")
            else:
                with open(f"File_Handling/Notes_app/Notes/{title.replace(' ', '_')}.txt", "a") as f:
                    f.write("\n" + note)
                print("Note written.")
        else:
        ...
        if title.lower() == "e":
            break
        if not (os.path.exists(f"File_Handling/Notes_app/Notes/{title}.txt")):
            print("File doesnt exist.")
        else:
            with open(f"File_Handling/Notes_app/Notes/{title.replace(' ', '_')}.txt") as f:
                print(f.read())
    elif action == "3":
        lines_delete = []
        delete_which = input(
        ...
        if delete_which == "1":
            try:
                ...
                if not (os.path.exists(f"File_Handling/Notes_app/Notes/{title}.txt")):
                    ...
                    with open(f"File_Handling/Notes_app/Notes/{title}.txt", "r") as f:
                        lines = f.readlines()
                ...
                with open(f"File_Handling/Notes_app/Notes/{title}.txt", "w") as f:
                    for line in lines:
                        f.write(line)
            except ValueError:
            ...
            title = input(
                "Enter title of your note file: ").strip().title().replace(" ", "_")
            if os.path.exists(f"File_Handling/Notes_app/Notes/{title}.txt"):
                os.remove(f"File_Handling/Notes_app/Notes/{title}.txt")
            else:
